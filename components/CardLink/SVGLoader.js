...
class SVGLoader extends Loader {
	...
	parse( text ) {
		...
		function parseNode( node, style, root ) {
    ...
    console.log(xml.documentElement.nodeType);

    // global.window.svgLoader = global?.window?.svgLoader ?? {}
    const containerElement = document.createElement('div')
    const key = text
    containerElement.style.display = 'none'
    containerElement.attachShadow({ mode: "open" })
    containerElement.shadowRoot.innerHTML = text
    // global.window.svgLoader[key] = containerElement

    document.body.appendChild(containerElement)

		parseNode(containerElement.shadowRoot.children[0], {
			fill: '#000',
			fillOpacity: 1,
			strokeOpacity: 1,
			strokeWidth: 1,

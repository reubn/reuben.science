...
This **hack** works around that, patching the `URL.toString()` function so that the `URL` points to the chunk, emitted to the `public` folder - which is served at the server root.

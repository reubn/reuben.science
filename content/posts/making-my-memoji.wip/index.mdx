...
The fact that they're fully rigged, and tightly integrated with iOS devices' built-in face tracking technology makes them really ease to create, and perfect for the majority who don't possess the artistic aptitude to use Blender for such tasks. This can look really great[^josh] however, and should be the default first choice for anyone who's able to.
...
A simpler option might be to combine the frames into a sprite sheet. I've had a play with this, and I've only been able to reduce the total size to about `715KB`. I still need to test if this meagre compression benefit outweighs the speed benefits that we achieve by serving individual frames concurrently over `HTTP2`.
